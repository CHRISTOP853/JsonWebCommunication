...
private void LoadTeam_Click(object sender, RoutedEventArgs e)
{
    ...
    private void GraphStats_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            if (dataGrid.ItemsSource is null)
                throw new InvalidOperationException("No data loaded in the grid to graph.");

            var stats = dataGrid.ItemsSource.Cast<GameStats>().ToList();
            var graph = new SeasonStatsGraph(stats)
            {
                Owner = this, // set owner so CenterOwner works
                WindowStartupLocation = WindowStartupLocation.CenterOwner
            }; // instantiate graph window with stats data
            graph.Show(); // display graph

            SetStatus($"Extracted {stats.Count} game stats and displayed graph."); // update status bar
        }
        catch (Exception ex)
        {
            ShowError("Graphing failed", ex);
